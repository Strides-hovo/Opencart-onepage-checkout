...
class ControllerSaleOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			if ( $str_invoices = json_decode($order_info['custom_field'], true)  ) {
				 $data['_str_invoice_text'] = <<<TEXT
					<div><strong> Invoice </strong>: <span> {$str_invoices['tax_office']}</span></div><br />
					<div><strong> Number of vat </strong>: <span> {$str_invoices['number_of_vat']}</span></div> <br />
					<div><strong> Activity </strong> <span>: {$str_invoices['activity']}</span></div>
				TEXT;
			}
